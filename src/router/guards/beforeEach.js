 ...
 * 获取菜单数据
 */
async function getMenuData(router) {
  try {
 ...
 * 处理前端控制模式的菜单逻辑
 */
async function processFrontendMenu(router) {
 ...
 * 处理后端控制模式的菜单逻辑
 */
async function processBackendMenu(router) {
 ...
 * 递归过滤空菜单项
 */
function filterEmptyMenus(menuList) {
  return menuList
    .map((item) => {
      // 如果有子菜单，先递归过滤子菜单
 ...
 * 注册路由并存储菜单数据
 */
async function registerAndStoreMenu(router, menuList) {
 ...
 * 验证菜单列表是否有效
 */
function isValidMenuList(menuList) {
